...
export default function ReadERC721(props: Props) {
  ...
  const { data: name } = useSWR([addressContract, 'name'], {
    fetcher: fetcher(library, abi),
  })
  ...
  //     console.log('Transfer|received', { from, to, tokenId: tokenId.toString(), event })
  //     mutate(undefined, true)
  //   })
  ...
  return (
    <div>
      ...
      <Text my={4}>{name} token in current account: {balance
        ? balance.toString()
        : " "
      } {symbol}</Text>
    </div>
